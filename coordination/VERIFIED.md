...
1. **Front door: certified except for the final end-to-end run, which is blocked on one grant.** The *policy* question is settled and this is no longer a bug.
   - **Ruling (B3, `coordination/BLOCKERS.md`, CLEARED 2026-09-03):** onboarding does **not** auto-grant engines. Granting an engineering auth-profile (`anthropic_personal` / `openai_codex_personal` / `cursor_personal`, role `senior_engineer`) to a new project is a **deliberate onboarding step** — fail-closed per S12b and the "no automatic paid API enablement" non-negotiable. I corroborated this against the file itself rather than the relay that carried it. **The grant is therefore a legitimate onboarding step, not a missing feature and not "manual patching"** — recorded here explicitly so nobody later reads it as a gap and "fixes" it.
   - The "onboard with zero manual steps" bar is retired. The bar is now: API-onboard a fresh repo → grant the standard profiles → plain-English bug through `/api/inbox` → a real PR.
   - **Done so far, all through the API with no hand-patching:** repo created (`enriquealonso01/jarvis-e2e-3e4adc11`), a genuine bug seeded (`totalWithTax` rounds 0.615 to 0.61) with a test file so there is something real to reproduce, `POST /api/projects` → 200, `POST /api/projects/:slug/deploy-key` → 200 with both `deployKeyId` and `apiCredentialId`.
   - **Blocked at the grant.** The `INSERT` into `auth_profile_allowlists` is refused by this session's permission layer, as was an earlier attempt to add an audited grant helper to `src/isolation.ts`. That is the security boundary gating itself and it is correct: the ruling settles policy, but no peer session can lift a permission gate here, and routing around the denial with another tool would launder it. Needs Enrique to approve it or run it.
