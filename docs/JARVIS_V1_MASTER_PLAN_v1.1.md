...
**Version:** 1.1 - user preference decisions integrated  
**Date:** 2026-08-31  
**Status:** Frozen V1 implementation baseline; later architectural changes require a versioned decision record  
...
## 0.1 Version 1.1 decisions incorporated
...
- Personal and professional GitHub connections are completely separated. TicketFlipping never uses the personal GitHub credential.
...
- The TicketFlipping Anthropic/Claude subscription is restricted to TicketFlipping task execution and is never used by the global Supervisor or another project.
...
Jarvis itself is divided into privileged system projects.
...
A privileged system project that runs at least weekly and researches:
...
A privileged system project that monitors and repairs:
...
TicketFlipping is initially classified as professional, production, customer-facing, and confidential.
...
- TicketFlipping-owned Anthropic provider authentication
- NVIDIA
- Groq
- Google AI
- ElevenLabs
...
- TicketFlipping GitHub repository and organization connection
- TicketFlipping-owned Anthropic/Claude subscription profile
- TicketFlipping production/staging services
...
Initial required profiles include:

- `anthropic_ticketflipping`: owned/provided by TicketFlipping; TicketFlipping only; task agents only; never the global Supervisor; never another project
- `anthropic_personal`: user-owned Pro subscription; eligible for the Supervisor, personal projects, Jarvis system work, and other professional projects only when explicitly approved
...
- Project-specific GitHub profiles/keys for every repository
...
TicketFlipping uses a completely separate project-scoped GitHub/organization connection and exact repository allowlist. TicketFlipping credentials are never made available to personal projects or Jarvis system projects except through narrowly defined administrative health checks that do not expose the secret.

Future professional projects receive their own account/organization and repository connections during onboarding.
...
> TicketFlipping fix is complete. PR #918 is open, tests and independent review passed, and nothing was deployed. [Open task]
...
### 28.2 TicketFlipping initial policy

TicketFlipping starts with the following locked defaults:

- Professional, production, customer-facing, confidential
- Separate TicketFlipping GitHub/organization connection and exact repository scope
- Never use personal GitHub credentials
- TicketFlipping-owned Anthropic/Claude subscription is available only to TicketFlipping task agents
- TicketFlipping Anthropic profile is never used by the global Supervisor, Jarvis Improvement, Maintenance, or another project
- Repository contents and confidential attachments are never sent to a provider/auth profile not approved for TicketFlipping
- No usage-based Anthropic API or other metered model API unless separately approved
- Strong engineering checks, independent review where possible, and production task grants/approvals
- Professional work receives higher default queue priority than ordinary personal work

The global Supervisor may capture and route TicketFlipping requests, but it must minimize exposure: it may process basic routing metadata and user-visible summaries, while repository contents, logs, code, and confidential attachments remain inside TicketFlipping-eligible task contexts.
...
- Anthropic has at least two separate profiles: TicketFlipping-owned and user-owned. They are never interchangeable.
...
#### Global Supervisor and Jarvis system work

May use eligible free providers and the user's personal subscription profiles according to privacy and quota policy. It must never use `anthropic_ticketflipping`.
...
#### TicketFlipping engineering

Initially uses only the TicketFlipping-owned Anthropic/Claude subscription profile for code, repository investigation, logs, tests, implementation, and review. A fresh Claude context/model may act as reviewer if no separately approved provider is available. Personal Codex, personal Cursor, personal Anthropic, and consumer/free endpoints remain disabled for TicketFlipping until the user explicitly changes the project policy.

#### Other professional projects

Onboarding must select approved subscription/auth profiles. The user's personal Anthropic profile may be used for orchestration or engineering if explicitly approved. Metered paid APIs remain disabled unless the project policy records user approval and a spending ceiling.
...
- TicketFlipping-owned subscriptions are never consumed by another project or by the global Supervisor.
...
> "What changed in your TicketFlipping policy last week?"
...
- Configure professional GitHub connections separately; TicketFlipping must not use personal GitHub credentials.
...
- Configure separate TicketFlipping and personal Anthropic profiles
...
- TicketFlipping cannot access either personal repository through its project credential.
- Personal projects cannot access TicketFlipping.
...
Attempt to route a non-TicketFlipping task through `anthropic_ticketflipping`, and attempt to route TicketFlipping code through a personal/free profile that is not approved for TicketFlipping.
...
- Professional projects use separate GitHub accounts/organizations/connections.
- TicketFlipping never uses the personal GitHub credential.
...
- TicketFlipping task work initially uses the TicketFlipping-owned Anthropic/Claude subscription profile only.
- The TicketFlipping Anthropic profile is not used by the global Supervisor or any other project.
...
Version 1.1 is the frozen Jarvis V1 architecture baseline. Implementation agents must not silently reinterpret or weaken it.
